...
func (m *mockCredentialRepo) GetAll() ([]models.Credential, error) { return m.getAllFn() }
...
func newCredentialSvc(repo *mockCredentialRepo) services.CredentialService {
...
func TestGetPaginated_ReturnsCorrectPage(t *testing.T) {
	...
	result, err := newCredentialSvc(repo).GetPaginated(page, pageSize)
	if err != nil {
...
func TestGetPaginated_TotalPagesCalculation(t *testing.T) {
	cases := []struct {
		...
		{total: 11, pageSize: 5, wantTotalPges: 3}, // remainder → ceil
	...
	for _, tc := range cases {
		...
		t.Run("", func(t *testing.T) {
			...
			result, err := newCredentialSvc(repo).GetPaginated(1, tc.pageSize)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
...
func TestGetPaginated_DefaultsInvalidPage(t *testing.T) {
	...
	_, err := newCredentialSvc(repo).GetPaginated(0, 10)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if calledPage != 1 {
...
func TestGetPaginated_DefaultsInvalidPageSize(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run("", func(t *testing.T) {
			...
			_, err := newCredentialSvc(repo).GetPaginated(1, tc.input)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
...
func TestGetPaginated_PropagatesRepoError(t *testing.T) {
	...
	_, err := newCredentialSvc(repo).GetPaginated(1, 10)
